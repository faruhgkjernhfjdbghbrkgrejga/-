...
@st.cache_data
def process_file(uploaded_file=None, text_area_content=None, url_area_content=None):
    text_content = None
...
def quiz_creation_page():
    ...
    if upload_option in ["이미지 파일", "PDF 파일"]:
        text_content = process_file(uploaded_file=uploaded_file)
    elif upload_option == "직접 입력":
        text_content = process_file(text_area_content=text_area_content)
    elif upload_option == "URL":
        text_content = process_file(url_area_content=url_area_content)
    else:
